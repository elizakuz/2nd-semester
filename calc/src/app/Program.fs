...
let MapIt (list : Token list, t : list<string * double>) =
    ...
    for i = (list.Length - 1) downto 0 do//идем в обратную сторону - от конца к началу
        match list.[i] with
        ...
        | LB -> if sk > 0 then sk <- sk - 1
                else raise WrongExpr
        | RB -> sk <- sk + 1
...
let createTArr (list : Map list) =
    ...
    for i = 0 to list.Length - 1 do
        ...
        | MNum x -> t.[i] <- Val x
                    num <- num + 1
                    ind <- false
        | MOp (x, y) -> if ind then raise WrongExpr 
                        else t.[i] <- Oper (x, Nil, Nil, y)
                             op <- op + 1
                             ind <- true 
    if op = num - 1 then t
    else raise WrongExpr
